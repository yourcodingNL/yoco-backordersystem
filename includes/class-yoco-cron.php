<?php
/**
 * YoCo Cron Handler - FIXED: Prevents spam scheduling
 */
...
class YoCo_Cron {
    ...
    public static function init() {
        ...
        // Only check scheduling on admin pages, not every frontend pageview
        add_action('admin_init', array(__CLASS__, 'maybe_schedule_actions'), 20);
    ...
    public static function maybe_schedule_actions() {
        ...
        // Transient lock: prevent spam scheduling (1 minute cooldown)
        $lock_key = 'yoco_scheduling_lock';
        if (get_transient($lock_key)) {
            return; // Already scheduled in last minute, skip
        }
        ...
        if (!$cron_enabled) {
            ...
            error_log("YOCO CRON: Sync disabled - all actions removed");
            return;
        }
    ...
    private static function schedule_daily_syncs() {
        // ALWAYS clean slate: verwijder ALLE oude actions
        ...
        error_log("YOCO CRON: Cleared all existing scheduled actions");
    ...
    private static function schedule_test_mode() {
        ...
        error_log("YOCO CRON: Cleared all existing scheduled actions for test mode");
    ...
    public static function unschedule_all_actions() {
        ...
        as_unschedule_all_actions(self::DAILY_SYNC_HOOK, array(), self::GROUP);
        as_unschedule_all_actions(self::TEST_SYNC_HOOK, array(), self::GROUP);
        error_log("YOCO CRON: All actions unscheduled");
    }
