...
class YoCo_Sync {
    ...
    private static function process_supplier_feed($supplier_term_id, $settings) {
        ...
        foreach ($products as $post) {
            try {
                ...
                // Update stock
                if (self::update_supplier_stock($post->ID, $supplier_term_id, $stock_info, $sku, $ean)) {
                    $updated++;
                    
                    // Update product backorder status
                    YoCo_Product::update_product_backorder_status($post->ID);
                }
                
            } catch (Exception $e) {
                $errors[] = sprintf(__('Product ID %d: %s', 'yoco-backorder'), $post->ID, $e->getMessage());
            }
        }
    ...
    private static function parse_stock_info($row, $stock_column) {
        ...
        if (isset($row[$stock_column])) {
            $stock_quantity = intval($row[$stock_column]);
        }
