                    ...
                    <tr>
                        ...
                        <td data-colname="<?php esc_attr_e('Products', 'yoco-backorder'); ?>">
                            <?php if ($log['products_processed'] > 0): ?>
                                <div><?php echo sprintf(__('Processed: %d', 'yoco-backorder'), $log['products_processed']); ?></div>
...
jQuery(document).ready(function($) {
    ...
    $('#clear-old-logs').on('click', function() {
        if (!confirm('<?php esc_js(_e("Remove logs older than 30 days?", "yoco-backorder")); ?>')) {
            return;
        }
        ...
        setTimeout(function() {
            ...
            alert('<?php esc_js(_e("Old logs cleared", "yoco-backorder")); ?>');
            location.reload();
        }, 2000);
    });
});
